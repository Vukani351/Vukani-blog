            ...
            foundShips.map((ship, index) => (
              <div key={index}>
                <div className="mx-2 grid max-w-sm rounded ">
                  <img
                    src="https://i.imgur.com/Wr1slIc.png"
                  ...
                  <div className="px-2 my-4">
                    <h3 className="uppercase font-bold text-left text-xl mb-2">
                      {ship.name}{' '}
                    </h3>
                  </div>
                  <div className="px-2 text-left pt-2 pb-2">
                    {ship.mass_lbs && (
                      <p className="uppercase font-bold block rounded-full px-3 py-1 text-sm font-semibold text-black mr-2 mb-2">
                        mass: {ship.mass_lbs} lbs
                      </p>
                    )}
                    {!ship.mass_lbs && (
                      <p className="uppercase font-bold block rounded-full px-3 py-1 text-sm font-semibold text-red-300 mr-2 mb-2">
                        mass Not recorded
                      </p>
                    )}
                    <p className="uppercase font-bold block rounded-full px-3 py-1 text-sm font-semibold text-black mr-2 mb-2">
                  ...
                  <button className="h-12 uppercase mb-4 justify-self-center bg-black hover:bg-blue-700 text-white font-bold py-2 rounded w-11/12">
                    {' '}
                    learn more{' '}
                  </button>
                </div>
                ...
                return (
                  <div key={index}>
                    <div className="mx-2 grid max-w-sm rounded">
                      ...
                      {/* <img  onError={({ currentTarget }) => { currentTarget.src="https://farm5.staticflickr.com/4599/38583829295_581f34dd84_b.jpg";}} src={ship.image} className="img rounded-lg shadow-xl" alt='Ship - spaceX'style={{height: "10rem"}}  /> */}
                      <div className="px-2 my-4">
                        <h3 className="capitalize text-left font-bold text-xl mb-2">
                          {ship.name} {index}
                        </h3>
                      </div>
                      <div className="px-2 text-left pt-2 pb-2">
...
const ShipsComponent = () => {
  const { loading, error, data } = useQuery(['ships'], () => {
    return axios.get('https://api.spacexdata.com/v4/ships');
  });
